...
class Header(ctypes.LittleEndianStructure):
    '''
    Each packet carries different types of data rather than having one packet which
    contains everything.
    All packets are encoded using Little Endian format.
    Interpret the incoming data in the correct way
    '''
    _pack = 1
    _fields = [
        ('m_packetFormat',ctypes.c_uint16 ),        # 2022
        ('m_gameMajorVersion',ctypes.c_uint8),      # game major version     
        ('m_gameMinorVersion',ctypes.c_uint8),      # Game minor version
        ('m_packetVersion',ctypes.c_uint8),         # Version of this packet
        ('m_packetId',ctypes.c_uint8),              
        ('m_sessionUID',ctypes.c_uint64),           # Unique Identifier
        ('m_sessionTime',ctypes.c_float),           # Session timestamp
        ('m_frameIdentifier',ctypes.c_int32),       # Identifier for the frame data
        ('m_playerCarIndex',ctypes.c_uint8)         # index of player's car
    ]
...
class LapData(ctypes.LittleEndianStructure):
    ...
    _pack = 1
    _fields = [
        ('m_lastLapTimeInMS', ctypes.c_uint32),        # Last lap time in seconds
        ('m_currentLapTimeInMS', ctypes.c_float),     # Current time around the lap in seconds
        ('m_sector3TimeInMS', ctypes.c_uint16),        # Best lap time of the session in seconds
        ('m_sector1TimeInMS', ctypes.c_uint16),        # Sector 1 time in seconds
...
class CarSetupData(ctypes.LittleEndianStructure):
    ...
    _fields_ = [
        ...
        ('m_frontLeftTyrePressure', ctypes.c_float),      # Front tyre pressure (PSI)
        ('m_frontRightTyrePressure', ctypes.c_float),
        ('m_rearLeftTyrePressure', ctypes.c_float),       # Rear tyre pressure (PSI)
...
class PacketCarTelemetryData(ctypes.LittleEndianStructure):
    ...
    _fields_ = [
        ...
        ('m_mfdPanelIndex', ctypes.c_uint8)                # Bit flags specifying which buttons are being pressed
...
class CarStatusData(ctypes.LittleEndianStructure):
    ...
    _fields_ = [
        ...
        ('m_vehicleFiaFlags', ctypes.c_uint8),           # -1 = invalid/unknown, 0 = none, 1 = green, 2 = blue
                                                        #  3 = yellow, 4 = red
...
class FinalClassificationData(ctypes.LittleEndianStructure):
    ...
    _fields = [
        ...
        ('m_totalRaceTime',ctypes.c_float),
        ('m_penaltiesTime',ctypes.c_uint8),
        ('m_numPenalties', ctypes.c_uint8),
        ('m_numTyreStints',ctypes.c_uint8),
        ('m_tyreStintsActual[8]',ctypes.c_uint8),
        ('m_tyreStintsVisual[8]',ctypes.c_uint8),
        ('m_tyreStintsEndLaps[8]',ctypes.c_uint8)
    ]
...
class PacketFinalClassificationData(ctypes.LittleEndianStructure):
    ...
    _fields = [
        ...
        ('m_classificationData',FinalClassificationData * 20)
    ]
